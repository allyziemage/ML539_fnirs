...
def columnEnsembleMethod(classifier_list,Xtr,ytr,Xte,yte,clf_parameters=[]):
    #generate tuples (and format accordingly for the ensembler)
    estimator_list=[]
    Xtrain, Xtest, ytrain, ytest= Xtr, Xte, ytr, yte
...
def main():
    ...
    if not check_headers(data['testPath']):
        # adding small scripts to fix the headers if wrong format
        data['testPath'] = update_headers(data['testPath'])
    ...
    for job in data['jobs']:
        ...
        if(job['method']=='COLUMN_ENSEMBLE'):
            acc=columnEnsembleMethod(job['ensembleInfo'],Xtr,ytr,Xte, yte,clf_parameters=params)
        else:
